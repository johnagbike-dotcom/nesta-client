...
export function AuthProvider({ children }) {
  ...
  // Helper to (re)load Firestore profile
  ...
  const sendMfaCode = useCallback(async ({ recaptchaContainerId, hintUid } = {}) => {
    ...
    try {
      ...
      setMfaError(e?.message?.replace(/^Firebase:\s*/i, "") || "Could not send MFA code.");
      return { ok: false };
    } finally {
      setMfaPending(false);
    }
  ...
  const verifyMfa = useCallback(async (code) => {
    ...
    try {
      ...
      const cred = PhoneAuthProvider.credential(mfaVerificationId, String(code || "").trim());
      ...
      setMfaError(e?.message?.replace(/^Firebase:\s*/i, "") || "Invalid code. Try again.");
      return { ok: false };
    } finally {
      setMfaPending(false);
    }
  ...
  const startMfaEnrollment = useCallback(async (phoneNumber, recaptchaContainerId = "enroll-recaptcha") => {
    setMfaError("");
    if (!auth.currentUser) {
    ...
    try {
      ...
      setMfaError(e?.message?.replace(/^Firebase:\s*/i, "") || "Could not start MFA enrollment.");
      return { ok: false };
    } finally {
      setMfaPending(false);
    }
  ...
  const finishMfaEnrollment = useCallback(async (code, friendlyName = "My phone") => {
    setMfaError("");
    ...
    try {
      ...
      setMfaError(e?.message?.replace(/^Firebase:\s*/i, "") || "Could not enable MFA.");
      return { ok: false };
    } finally {
      setMfaPending(false);
    }
  ...
  const unenrollMfa = useCallback(async (factorUid) => {
    ...
    try {
      ...
      setMfaError(e?.message?.replace(/^Firebase:\s*/i, "") || "Could not disable MFA.");
      return { ok: false };
    } finally {
      setMfaPending(false);
    }
