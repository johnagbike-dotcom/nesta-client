...
/* ─── Password policy helpers (unchanged — all logic preserved) ─── */
const MIN_LEN   = 10;
...
export default function SignUpPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const existing = await getDoc(doc(db, "users", cleanEmail));
      if (existing.exists()) { setError("An account with this email already exists. Please sign in."); setSubmitting(false); return; }
      ...
      else if (code === "auth/weak-password")    { setError("Password too weak. Use at least 10 characters."); }
      else setError(stripFirebasePrefix(err?.message) || "Sign-up failed. Please try again.");
    } finally {
      setSubmitting(false);
    }
  }
        ...
        <div style={{ width:"100%", maxWidth:460, margin:"0 auto", padding:"40px 16px" }}>
          ...
          <div style={cardStyle}>
            <div style={{ textAlign:"center", marginBottom:20 }}>
              ...
              <h1 style={{ fontFamily:CORMORANT, fontSize:28, fontWeight:600, color:"#f5f0e8", margin:"0 0 8px" }}>Check your inbox</h1>
            ...
            <div style={{ borderRadius:14, border:"1px solid rgba(201,168,76,0.20)", background:"rgba(201,168,76,0.06)", padding:"14px 16px", fontSize:13, color:"rgba(255,255,255,0.6)", lineHeight:1.65, marginBottom:20 }}>
              Open the link in that email, then return here to sign in. If you don't see it, check your spam folder.
            </div>
          ...
          <div style={{ textAlign:"center", marginTop:20 }}>
            <Link to="/" style={{ fontSize:13, color:"rgba(255,255,255,0.35)", textDecoration:"none", borderBottom:"1px solid rgba(255,255,255,0.12)" }}>← Back to NestaNg</Link>
          </div>
        </div>
      </main>
    );
      ...
      <main style={pageWrap}>
        <div style={{ width:"100%", maxWidth:480, margin:"0 auto", padding:"32px 16px 48px" }}>
          ...
          <div style={{ textAlign:"center", marginBottom:24 }}>
            ...
            <h1 style={{ fontFamily:CORMORANT, fontSize:30, fontWeight:600, color:"#f5f0e8", margin:"0 0 6px" }}>Create your account</h1>
            <p style={{ fontSize:14, fontWeight:300, color:"rgba(255,255,255,0.45)" }}>Join Nigeria's premium short-let marketplace</p>
          </div>
          ...
          <div style={cardStyle}>
            ...
            <form onSubmit={handleSubmit} style={{ display:"grid", gap:14 }}>
              ...
              <label style={{ display:"grid", gap:6 }}>
                ...
                <input className="su-input" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your full name" autoComplete="name" required />
              </label>

              {/* Email */}
              <label style={{ display:"grid", gap:6 }}>
                <span style={labelStyle}>Email</span>
                <input className="su-input" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" autoComplete="email" required />
              </label>

              {/* Phone */}
              <label style={{ display:"grid", gap:6 }}>
                <span style={labelStyle}>Phone <span style={{ color:"rgba(255,255,255,0.25)", fontWeight:300 }}>(optional)</span></span>
                <input className="su-input" type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+234 800 000 0000" autoComplete="tel" />
              </label>
              ...
              <div style={{ display:"grid", gap:6 }}>
                ...
                <div style={{ position:"relative" }}>
                  ...
                  <button
                    ...
                    style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", fontSize:11, padding:"4px 10px", borderRadius:8, cursor:"pointer", border:"1px solid rgba(255,255,255,0.10)", background:"rgba(255,255,255,0.06)", color:"rgba(255,255,255,0.6)" }}
                ...
                {pwTouched && password && (
                  <div>
                    <div style={{ height:3, borderRadius:999, background:"rgba(255,255,255,0.08)", overflow:"hidden" }}>
                      <div style={{ height:"100%", width:`${(strengthScore/5)*100}%`, background:strengthColor, borderRadius:999, transition:"width 0.3s, background 0.3s" }} />
                    </div>
                ...
                {pwTouched && (
                  <div style={{ borderRadius:14, border:"1px solid rgba(255,255,255,0.07)", background:"rgba(255,255,255,0.03)", padding:"12px 14px", display:"grid", gap:7 }}>
              ...
              <label style={{ display:"flex", alignItems:"flex-start", gap:10, cursor:"pointer" }}>
                <input type="checkbox" className="su-check" checked={agreed} onChange={(e) => setAgreed(e.target.checked)} required />
                <span style={{ fontSize:13, color:"rgba(255,255,255,0.55)", lineHeight:1.65, marginTop:1 }}>
                  I agree to NestaNg's{" "}
                  <Link to="/terms"    target="_blank" style={{ color:"#c9a84c", textDecoration:"none", borderBottom:"1px solid rgba(201,168,76,0.3)" }}>Terms of Use</Link>
                  {" "}and{" "}
                  <Link to="/privacy"  target="_blank" style={{ color:"#c9a84c", textDecoration:"none", borderBottom:"1px solid rgba(201,168,76,0.3)" }}>Privacy Policy</Link>.
                </span>
              </label>
            ...
            <div style={{ marginTop:20, textAlign:"center", fontSize:13 }}>
              ...
              <Link to="/login" style={{ color:"#c9a84c", textDecoration:"none", borderBottom:"1px solid rgba(201,168,76,0.3)" }}>Sign in</Link>
            </div>
          </div>
