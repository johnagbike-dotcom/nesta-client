...
function CardStat({ label, value, helper, currency = false, tone, onClick }) {
  ...
  const toneClasses =
    tone === "amber" ? "border-amber-400/40 bg-amber-500/10" : "border-white/10 bg-white/5";
  ...
  return (
    <div
      ...
      className={`rounded-2xl px-4 py-3 border ${toneClasses} flex flex-col justify-between min-h-[84px] transition-all ${
        clickable ? "cursor-pointer hover:bg-white/10 hover:border-amber-300/60" : ""
      }`}
    ...
    >
      ...
      <div className="mt-1 text-2xl font-semibold">{Number.isNaN(safeValue) ? (currency ? "₦0" : "0") : formatted}</div>
      ...
      try {
        ...
        snap.forEach((docu) => {
          ...
          if (["confirmed", "completed", "paid"].includes(s)) {
            ...
            net += hostTake;
            bookingsConfirmed++;
          } else if (s === "pending" || s === "awaiting" || s === "hold" || s === "reserved_unpaid") {
            bookingsPending++;
          } else if (s === "cancelled") {
            cancelled++;
          } else if (s === "refunded") {
  ...
  return (
    <main className="min-h-screen bg-[#05070a] pt-20 pb-12 px-4 text-white">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {/* DEBUG LINE (helps confirm it’s not a “cap”) */}
        ...
        <header className="mt-2 flex flex-col md:flex-row md:items-end md:justify-between gap-3">
          <div>
            <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight text-amber-300">Host Dashboard</h1>
        ...
        {!isKycApproved && (
          <section className="rounded-2xl border border-amber-400/40 bg-amber-500/10 px-4 py-3 text-sm text-amber-50 flex items-center justify-between gap-3">
            <p>
              <span className="font-semibold">Finish your KYC</span> to unlock full visibility, payouts, and trust
              indicators on Nesta.
            </p>
            <button
        ...
        {err && (
          <div className="rounded-2xl border border-red-500/40 bg-red-500/10 p-3 text-sm text-red-100">{err}</div>
        )}

        {/* KPI GRID 1 */}
        <section className="grid gap-3 md:gap-4 md:grid-cols-3 lg:grid-cols-6">
          <CardStat label="Active listing(s)" value={kpis.active} helper="Live stays on Nesta" onClick={goManageListings} />
          <CardStat label="Confirmed bookings" value={rev.bookingsConfirmed} helper="Completed / paid" onClick={goReservations} />
          <CardStat label="Pending / upcoming" value={rev.bookingsPending} helper="Awaiting payment / arrival" onClick={goReservations} />
          <CardStat label="Nightly portfolio" currency value={nightlyPortfolio} helper="Across active listings" onClick={goManageListings} />
          <CardStat label="Gross revenue" currency value={rev.gross} helper="Before Nesta fees" onClick={goReservations} />
          <CardStat label="Host earnings (net)" currency value={rev.net} helper="After Nesta fee (est.)" onClick={goReservations} />
        </section>

        {/* KPI GRID 2 */}
        <section className="grid gap-3 md:gap-4 md:grid-cols-2 lg:grid-cols-4">
          <CardStat label="Needs attention" value={rev.needsAttention} helper="Pending / cancelled / refunded" tone="amber" onClick={goReservations} />
          <CardStat label="Cancelled" value={rev.cancelled} onClick={goReservations} />
          <CardStat label="Refunded" value={rev.refunded} onClick={goReservations} />
          <CardStat label="KYC status" value={isKycApproved ? 1 : 0} helper={kycStatusRaw || (isKycApproved ? "approved" : "pending")} />
        </section>
        ...
        <section className="flex flex-wrap gap-3 items-center mt-1">
          ...
          <button
            ...
            className={`ml-auto px-4 py-2 rounded-xl text-sm font-semibold transition ${
              isKycApproved ? "bg-amber-500 text-black hover:bg-amber-400" : "bg-white/10 text-white/40 cursor-not-allowed"
            }`}
